...
public class Scene4_2_Mult_matrix_vectorController implements Initializable {
    private Scene scene;
    private Stage stage;
    ...
    public void generateMatrixVector() {
        ...
        if (matrixPane1 != null && matrixPane2 != null) {
            ...
            for (int y = 0; y < length; y++) {
                for (int x = 0; x < width; x++) {
                    ...
                    tf1.textProperty().addListener(
                            (observable, oldValue, newValue) -> matrix1[row1][column1] = Double.parseDouble(newValue));
                }
            }
            ...
            for (int z = 0; z < length; z++) {
                ...
                final int index = z;
                tf2.textProperty()
                        .addListener((observable, oldValue, newValue) -> vector[index] = Double.parseDouble(newValue));
            }
        }
    }
    ...
    void calculer(ActionEvent event) throws IOException {
        ...
        switch (selectedOperation) {
            ...
            case "TriangInf matrice * vecteur":
                if ((type.recognize(matrix1) != 2)) {
                    displayAlert("the  matrix should be of the type TriangInf ");
                } else
                    resultvector = multiply.tringinf(matrix1, vector);
                break;

            case "Dense * vecteur":

                resultvector = multiply.dense(matrix1, vector);
                break;

            case "TriangSup * vecteur":
                if ((type.recognize(matrix1) != 1)) {
                    displayAlert("the  matrix should be of the type TriangSup ");
                } else
                    resultvector = multiply.tringsup(matrix1, vector);

                break;

            case "BandeSup * vecteur":
                if ((type.recognize(matrix1) != 3)) {
                    displayAlert("the  matrix should be of the type BandeSup ");
                } else
                    resultvector = multiply.supdemibande(matrix1, vector);

                break;

            case "BandeInf * vecteur":
                if ((type.recognize(matrix1) != 4)) {
                    displayAlert("the  matrix should be of the type BandeInf ");
                } else
                    resultvector = multiply.infdemibande(matrix1, vector);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        choice.getItems().addAll("Dense * vecteur", "TriangInf matrice * vecteur", "TriangSup * vecteur",
                "BandeSup * vecteur", "BandeInf * vecteur");
    }
}
