...
public class scene5_IterativeResolution implements Initializable {
    private Scene scene;
    private Stage stage;
    @FXML
    ...
    public double getEpsilonFromTextField() {
        TextField epsilon = (TextField) scene.lookup("#epsilon");
        if (epsilon != null) {
            try {
    ...
    void calculer(ActionEvent event) throws IOException {
        ...
        double eps = getEpsilonFromTextField();

        switch (selectedOperation) {
            case "jacoubi avec epsilon":
            resultvector = resolution.jacobieEpsilon(matrix1, vector, eps);
                break;

            case "jacoubi avec nb iterations":
            resultvector = resolution.jacobieIteration(matrix1, vector, eps);
                break;

            case "Gaus-siede avec epsilonl":
            resultvector = resolution.GaussSeidelEpsilon(matrix1, vector, eps);
                break;

            case "Gaus-siedel avec nb iterations":
            resultvector = resolution.GaussSeidelIteration(matrix1, vector, eps);
                break;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        choice.getItems().addAll("jacoubi avec epsilon","jacoubi avec nb iterations","Gaus-siede avec epsilonl","Gaus-siedel avec nb iterations");  
      }
}
